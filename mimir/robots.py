...
# The migration that creates `robots_rules` seeds these same values;
# this constant is the single source of truth for `reset_rules`.
...
def render_robots_txt(session: Session, sitemap_url: str) -> str:
    ...
    between stanzas; trailing `Sitemap:` line."""
    out = StringIO()
    first = True
    for rule in list_rules(session):
        paths = rule.disallow_paths or []
        # Skip degenerate rows (no crawl_delay AND no disallow paths).
        # A row with only a User-agent line and nothing else is
        # nonsense and confuses some bots.
        if rule.crawl_delay is None and not paths:
            continue
        if not first:
            out.write("\n")
        first = False
