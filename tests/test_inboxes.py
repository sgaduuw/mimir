...
def test_delete_inbox_removes_orphan_articles(seeded_db):
    ...
    with seeded_db() as s:
        message_ids = sorted(mid for (mid,) in s.execute(select(Article.message_id)))
...
def test_bootstrap_inboxes_inserts_env_rows_then_no_ops_on_rerun(
    ...
    with seeded_db() as s:
        row = s.execute(select(Inbox).where(Inbox.name == "fresh-inbox")).scalar_one()
...
def test_bootstrap_inboxes_preserves_admin_managed_rows(
    ...
    with seeded_db() as s:
        names = sorted(n for (n,) in s.execute(select(Inbox.name)))
