...
def test_tracemalloc_interval_negative_treated_as_zero(monkeypatch):
    monkeypatch.setenv("TRACEMALLOC_INTERVAL_SECONDS", "-1")
    s = Settings(secret_key="x" * 32)
    # Negative is parsed but treated as "off" at the consumer (see
    # _maybe_start_tracemalloc_snapshotter). Type stays int; no
    # validation rejection here so a typo doesn't crash startup.
