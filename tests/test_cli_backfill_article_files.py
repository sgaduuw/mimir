...
def test_backfill_prefers_canonical_inbox_for_crossposts(seeded_db, tmp_path):
    ...
    # Ingest into beta so we can capture the real commit SHA.
    with SessionLocal() as s:
        beta = s.execute(select(Inbox).where(Inbox.name == "beta")).scalar_one()
        beta.mirror_path = str(beta_mirror)
        s.commit()
        ingest_epoch(s, beta, "0.git", epoch_path, workers=1)
        # Wipe the file rows ingest created so the backfill has work
        # and reads through the canonical-inbox decision.
        s.query(ArticleFile).delete()
        s.commit()

        # Now cross-post: add an alpha ArticleList for the same
        # article with a bogus SHA (alpha's mirror won't contain it).
        # Pin canonical_inbox to beta -- the assertion is that the
        # backfill follows that, not lists[0].
        alpha = s.execute(select(Inbox).where(Inbox.name == "alpha")).scalar_one()
        article = s.execute(
    ...
    # alpha's mirror_path is the seeded /tmp/alpha which doesn't
    # exist; if the backfill follows article.lists[0] and that turns
    # out to be alpha, the read fails -> "skipped".
