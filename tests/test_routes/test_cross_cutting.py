...
def test_unmatched_404_renders_branded_html_page(client):
    """#237: errors render via `mimir/templates/error.html` extending
    base.html instead of Werkzeug's default plain-text fallback. The
    response carries the site shell (nav with the wordmark, footer
    crediting mimir + version), the status code stays 404, and the
    body is HTML, not text/plain.

    The unmatched-URL path is the most common error case (typos in
    shared links, scraper probes for `/wp-admin`, etc.), so this is
    the one to pin most tightly."""
    r = client.get("/no/such/route/exists/here")
    assert r.status_code == 404
    assert r.mimetype == "text/html"
    body = r.data.decode()
    # Site shell present.
    assert "<title>404 Not found" in body
    assert "<nav>" in body, "branded 404 missing site nav"
    # Footer credit (generator line via base.html).
    assert "Generated by" in body
    # And the error body itself is recognisable.
    assert "404 Not found" in body
    # Werkzeug's default body string is NOT what we serve.
    assert "<!doctype html" in body.lower()


def test_error_page_emits_noindex_meta_and_no_canonical(client):
    """Error responses must carry `<meta name="robots" content=
    "noindex">` so crawlers don't index typo URLs, and must NOT carry
    `<link rel="canonical">` (a canonical pointing at the error URL
    would tell search engines "this 404 is authoritative," which is
    exactly the wrong signal). Both are wired through the `noindex`
    flag in the `_inject_template_globals` context processor +
    `base.html`'s conditional emission.
    """
    r = client.get("/no/such/route/exists/here")
    assert r.status_code == 404
    body = r.data.decode()
    assert '<meta name="robots" content="noindex">' in body, (
        "error page missing noindex meta"
    )
    assert '<link rel="canonical"' not in body, (
        "error page must not emit canonical link"
    )


def test_inbox_scoped_404_also_renders_branded(client):
    """An `abort(404)` from inside a route handler (e.g. unknown
    inbox slug) also hits the branded handler, not Werkzeug's
    default. Guards against the case where someone wires
    `@app.errorhandler` at the blueprint level only and
    blueprint-internal aborts skip the branded path.
    """
    r = client.get("/nonexistent-inbox/")
    assert r.status_code == 404
    body = r.data.decode()
    assert "<title>404 Not found" in body
    assert '<meta name="robots" content="noindex">' in body
